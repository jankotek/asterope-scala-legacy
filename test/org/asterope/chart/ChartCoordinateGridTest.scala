...
class ChartCoordinateGridTest extends ScalaTestCase{
	...
	def testLayerNoGridJ2000{
		...
		assert(c.getLayer(ChartLayers.coordinateGridJ2000).getChildrenCount === 0)
	...
	def testLayerNoGridJ1950{
		...
		assert(c.getLayer(ChartLayers.coordinateGridJ1950).getChildrenCount === 0)
		painter.updateChart(c,config)
		assert(c.getLayer(ChartLayers.coordinateGridJ1950).getChildrenCount === 0)
	}
	...
	def testLayerNoGridGalactic{
		...
		assert(c.getLayer(ChartLayers.coordinateGridGalactic).getChildrenCount === 0)
		painter.updateChart(c,config)
		assert(c.getLayer(ChartLayers.coordinateGridGalactic).getChildrenCount === 0)
	}
	...
	def testLayerNoGridEcliptic{
		...
		assert(c.getLayer(ChartLayers.coordinateGridEcliptic).getChildrenCount === 0)
		painter.updateChart(c,config)
		assert(c.getLayer(ChartLayers.coordinateGridEcliptic).getChildrenCount === 0)
	}
